    ...
    return this._getCollection(
      ...
      function(err, collection) {
        ...
        return collection.updateOne(
          { owner_id: this._client.authedId() },
          { $set: obj },
          { upsert: true }
        );
      }.bind(this)
  ...
  send: function(eventName, metadata) {
    ...
    return this._getCollection(
      ...
      function(err, collection) {
        ...
        metadata.owner_id = this._client.authedId();
        debug('sending event `%s`', eventName, metadata);
        return collection.insertOne(metadata, { upsert: true });
      }.bind(this)
    );
  }
});
