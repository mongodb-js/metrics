...
var IntercomTracker = State.extend({
  ...
  derived: {
    ...
    enabledAndConfigured: {
      deps: ['enabled', 'app_id'],
      fn: function() {
        return this.enabled && this.app_id !== '';
      }
    }
  },
  _setup: function() {
    ...
    if (typeof ic === 'function') {
      ...
      function l() {
        ...
        s.async = true;
        s.src = format('https://widget.intercom.io/widget/%s', self.app_id);
  ...
  _enabledConfiguredChanged: function() {
    if (this.enabledAndConfigured) {
      this._setup();
      debug('adding change handler on user_id');
      this.on('change:user_id', this._updateIntercom);
    } else {
      this.off('change:user_id', this._updateIntercom);
      debug('removing change handler on user_id');
      this._teardown();
    }
  },
