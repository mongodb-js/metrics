...
module.exports = State.extend({
  idAttribute: 'id',
  id: 'Base',
  eventTrackers: ['ga', 'mixpanel', 'intercom', 'amplitude'],
  collections: {
  ...
  /**
   * send any Amplitude event. This is the only actual call to Amplitude.
   *
   ...
   * @api private
   */
  _send_amplitude: function(eventName, metadata, callback) {
    var tracker = this.trackers.get('amplitude');
    if (tracker && tracker.enabled) {
   ...
   * mixpanel, amplitude). Re-format resource and action into `<Resource> <action>` event
  ...
  _send_event: function(metadata, callback) {
    ...
    if (this.eventTrackers.indexOf('amplitude') !== -1) {
      tasks.amplitude = that._send_amplitude.bind(that, eventName, metadata);
    }
