...
module.exports = State.extend({
  ...
  _send_ga_event: function(options, callback) {
    var resource = options.resource || this.id;
    var action = options.action || callerId.getData().methodName;
  ...
  _send_intercom: function(eventName, metadata, callback) {
    ...
    if (callback) {
      return callback(new Error('Intercom tracker not set up.'));
    }
  },
  /**
   * prepare sending an event to intercom. Re-format resource and action into
   * `<Resource> <action>` string, attach all other relevant information
   * as metadata.
   *
  ...
  _send_intercom_event: function(options, callback) {
    var resource = _.get(options, 'resource', this.id);
    var action = _.get(options, 'action', callerId.getData().methodName);
    options = _.omit(options, ['resource', 'action']);
